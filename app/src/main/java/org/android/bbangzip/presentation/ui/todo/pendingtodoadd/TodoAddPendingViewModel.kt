...
constructor(
    ...
    override fun handleEvent(event: TodoAddPendingContract.TodoAddPendingEvent) {
        when (event) {
            ...
            TodoAddPendingContract.TodoAddPendingEvent.OnFilterBottomSheetDismissRequest -> updateState(
                TodoAddPendingContract.TodoAddPendingReduce.UpdateToDoFilterBottomSheetState(
                    todoFilterBottomSheetState = false
                )
            )

            TodoAddPendingContract.TodoAddPendingEvent.OnFilterIconClicked -> updateState(
                TodoAddPendingContract.TodoAddPendingReduce.UpdateToDoFilterBottomSheetState(
    ...
    override fun reduceState(state: TodoAddPendingContract.TodoAddPendingState, reduce: TodoAddPendingContract.TodoAddPendingReduce): TodoAddPendingContract.TodoAddPendingState {
        return when (reduce) {
            TodoAddPendingContract.TodoAddPendingReduce.ResetSelectedItemList -> state.copy(
                selectedItemList = listOf()
            )

            is TodoAddPendingContract.TodoAddPendingReduce.UpdateCardState -> state.copy(
                todoList = state.todoList.map { item ->
                    if (item.pieceId == reduce.pieceId) item.copy(cardState = reduce.cardState) else item
                }
            )

            is TodoAddPendingContract.TodoAddPendingReduce.UpdateFilterType -> state.copy(
                selectedFilter = reduce.selectedFilter
            )

            is TodoAddPendingContract.TodoAddPendingReduce.UpdateSelectedItemList -> state.copy(
                selectedItemList = state.selectedItemList.plus(reduce.pieceId)
            )

            is TodoAddPendingContract.TodoAddPendingReduce.UpdateToDoFilterBottomSheetState -> state.copy(
                todoFilterBottomSheetState = reduce.todoFilterBottomSheetState
            )

            is TodoAddPendingContract.TodoAddPendingReduce.DeleteSelectedItemList -> state.copy(
