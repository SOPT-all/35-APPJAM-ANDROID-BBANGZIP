...
constructor(
    ...
    override fun handleEvent(event: MyBadgeCategoryContract.MyBadgeCategoryEvent) {
        when (event) {
            ...
            MyBadgeCategoryContract.MyBadgeCategoryEvent.OnBadgeDetailBottomSheetDismissRequest ->  updateState(
                MyBadgeCategoryContract.MyBadgeCategoryReduce.UpdateBadgeDetailBottomSheetState(
                    badgeDetailBottomSheetState = false
                )
            )
    ...
    override fun reduceState(
       ...
       return when (reduce) {
            is MyBadgeCategoryContract.MyBadgeCategoryReduce.UpdateBadgeCategoryList -> state.copy(
                badgeCategoryList1 = reduce.badgeCategoryList1,
                badgeCategoryList2 = reduce.badgeCategoryList2
            )

            is MyBadgeCategoryContract.MyBadgeCategoryReduce.UpdateBadgeDetail -> state.copy(
                badgeDetail = reduce.badgeDetail
            )

            is MyBadgeCategoryContract.MyBadgeCategoryReduce.UpdateBadgeDetailBottomSheetState -> state.copy(
